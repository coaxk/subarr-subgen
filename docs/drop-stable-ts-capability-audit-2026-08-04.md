...
Generated by `scripts/capability_audit.py`. Every hunk in `patches/` is offered to a tree at each revision by two probes -- one standalone, one cumulative -- and judged by whether `git apply` accepts it.
...
- **Standalone probe**: INTACT 31, BROKEN_BY_BRANCH 11, INCONCLUSIVE 86, SKIPPED_OTHER_FILE 1
- **Resolved, both probes**: INTACT 76, BROKEN_BY_BRANCH 11, CASCADE 42
- **Base cumulative pass**: 129/129 applied in `patches/series` order (`BASE_ANOMALY` 0)

## Method: two probes, cross-tabulated
...
**Probe 1, standalone.** Each hunk is reconstructed as a one-hunk patch and offered to a *pristine* tree at each revision. It answers one question: does the text this hunk lands on exist in that tree?

| Applies to base | Applies to branch | Reading |
| --- | --- | --- |
| yes | yes | the code this hunk lands on still exists |
| yes | no | decisive: the branch removed it |
| no | either | unreadable on its own -- the anchor is text an **earlier patch of ours** creates, which a pristine tree never has |

That last row is the majority of the stack, which is why this probe alone could only ever report a floor.

**Probe 2, cumulative.** A second pair of trees is built and every hunk is applied **in `patches/series` order**, mutating the tree as it goes and continuing past failures. It answers a different question: does the stack still land, in order? It sees the anchors our own patches create, so it is the only probe that can judge the hunks probe 1 cannot read -- and it cannot, on its own, tell "the branch deleted this" from "an earlier hunk of ours never got to add the line I needed".

Neither probe resolves a hunk alone. Cross-tabulated, they resolve all of them:

| Condition, read top to bottom | Outcome |
| --- | --- |
| the cumulative pass fails on the **base** | `BASE_ANOMALY` -- a tooling alarm, not a finding; see below |
| standalone: applies to base, not to branch | `BROKEN_BY_BRANCH` -- decisive, whatever the ordering |
| the cumulative pass lands it on the **branch** | `INTACT` |
| anything else | `CASCADE` -- its anchor was ours, and an earlier break took it |
...
Outcomes are the **resolved** verdicts -- both probes, cross-tabulated.

| Capability | Providing patches | Outcomes |
| --- | --- | --- |
| `asr_arena` | `0016-asr-path-and-kwargs.patch` | INTACT 6, BROKEN_BY_BRANCH 1, CASCADE 3 |
| `asr_detected_language` | `0018-asr-detected-language.patch` | INTACT 2, BROKEN_BY_BRANCH 2, CASCADE 1 |
| `asr_vanilla_base` | `0017-asr-vanilla-base.patch` | INTACT 6, CASCADE 3 |
| `async_config` | `0029-async-config-switch.patch` | INTACT 1, CASCADE 5 |
| `audio_language_override` | `0009-audio-language-override.patch` | INTACT 6, BROKEN_BY_BRANCH 1, CASCADE 1 |
| `concurrent_transcriptions` | `0023-expose-concurrent-transcriptions.patch` | CASCADE 1 |
| `curated_language_prompts` | `0012-curated-language-prompts.patch` | INTACT 1, CASCADE 2 |
| `detect_language_track` | `0027-config-live-model-and-swap-lock.patch` | INTACT 1, CASCADE 4 |
| `ignore_forced_subtitles` | `0022-runtime-config-endpoint.patch` | INTACT 1, CASCADE 3 |
| `per_request_kwargs` | `0014-per-request-kwargs.patch` | INTACT 7, CASCADE 3 |
| `per_request_task` | `0015-per-request-task.patch` | INTACT 5, CASCADE 2 |
| `queue_cancel` | `0010-queue-cancel.patch` | INTACT 2, CASCADE 2 |
| `request_ignore_forced` | `0024-per-request-ignore-forced.patch` | INTACT 12, CASCADE 2 |
| `robust_language_detection` | `0011-robust-language-detection.patch` | INTACT 1, CASCADE 2 |
| `runtime_config` | `0022-runtime-config-endpoint.patch` | INTACT 1, CASCADE 3 |
| `safe_decode_preset` | `0013-safe-decode-preset.patch` | INTACT 1, CASCADE 2 |
...
**1. The census is COMPLETE. `BROKEN_BY_BRANCH` is the full count of independent breaks, and `CASCADE` is attributable to it.** Every hunk the standalone probe returned `INCONCLUSIVE` on has been resolved by the cumulative probe, so no hunk is left unjudged and no number here is a floor. Read the two decisive outcomes like this:

- `BROKEN_BY_BRANCH` is **independent** damage. The hunk applies to the pinned base and not to the branch, which names upstream code the branch removed. Nothing about ordering can explain that away, so this count is the same under both probes.
- `CASCADE` is **attributable** damage, not additional damage. The hunk failed cumulatively on the branch while no probe blames the branch for the text it wanted: its anchor was a line one of our own earlier hunks adds, and that hunk is one of the breaks above. Repair a `BROKEN_BY_BRANCH` hunk and its cascade comes back within reach; count the two together and you count one wound twice.
- `INTACT` now includes every hunk the standalone probe could not read. That is the resolution the cumulative probe was built for, and it is why the resolved `INTACT` count is far larger than the standalone one.

The invariant that makes all of this readable is the base cumulative pass in the header: the stack applies to the pinned base in `patches/series` order with **zero** `BASE_ANOMALY`. That is true by construction -- it is what `patches/` *is* -- so a nonzero `BASE_ANOMALY` means the probe stopped reproducing the stack. Treat it as a tooling alarm and fix the tool before believing any other number in this report.
...
**3. `SKIPPED_OTHER_FILE` is a standalone-probe scope limit, not a gap.** The standalone probe judges only `subgen.py`, because that is the file this branch refactors; the hunk targeting `entrypoint.sh` (from `0021`) is reported as out of scope rather than dropped, so its denominator stays honest. Both trees nonetheless materialise `entrypoint.sh` as well -- they must, or that hunk would fail the base cumulative pass and raise a `BASE_ANOMALY` that says nothing about either revision -- so the resolved column judges it like any other.

## Whole-stack census, by patch

Every hunk in `patches/`, including patches that map to no advertised capability. This is the denominator the per-capability table above cannot show. Both probes are shown so the cross-tabulation can be checked patch by patch: read a row as *what one probe could see* against *what the pair concluded*.

| Patch | Hunks | Standalone probe | Resolved |
| --- | --- | --- | --- |
| `0001-per-language-kwargs.patch` | 4 | INTACT 1, BROKEN_BY_BRANCH 3 | INTACT 1, BROKEN_BY_BRANCH 3 |
| `0002-eager-model-load.patch` | 1 | INTACT 1 | INTACT 1 |
| `0003-reverse-sort-transcribe-existing.patch` | 2 | INTACT 1, INCONCLUSIVE 1 | INTACT 2 |
| `0004-batch-structured-response.patch` | 1 | INTACT 1 | INTACT 1 |
| `0005-gsq-return-str.patch` | 3 | INTACT 3 | INTACT 3 |
| `0006-fastapi-jsonresponse-import.patch` | 1 | BROKEN_BY_BRANCH 1 | BROKEN_BY_BRANCH 1 |
| `0007-deduplicated-queue-type-tracking-and-queue-endpoint.patch` | 5 | INTACT 4, BROKEN_BY_BRANCH 1 | INTACT 4, BROKEN_BY_BRANCH 1 |
| `0008-log-language-detection-probability.patch` | 2 | BROKEN_BY_BRANCH 2 | BROKEN_BY_BRANCH 2 |
| `0009-audio-language-override.patch` | 8 | INTACT 1, BROKEN_BY_BRANCH 1, INCONCLUSIVE 6 | INTACT 6, BROKEN_BY_BRANCH 1, CASCADE 1 |
| `0010-queue-cancel.patch` | 4 | INCONCLUSIVE 4 | INTACT 2, CASCADE 2 |
| `0011-robust-language-detection.patch` | 3 | INTACT 1, INCONCLUSIVE 2 | INTACT 1, CASCADE 2 |
| `0012-curated-language-prompts.patch` | 3 | INCONCLUSIVE 3 | INTACT 1, CASCADE 2 |
| `0013-safe-decode-preset.patch` | 3 | INCONCLUSIVE 3 | INTACT 1, CASCADE 2 |
| `0014-per-request-kwargs.patch` | 10 | INTACT 2, INCONCLUSIVE 8 | INTACT 7, CASCADE 3 |
| `0015-per-request-task.patch` | 7 | INCONCLUSIVE 7 | INTACT 5, CASCADE 2 |
| `0016-asr-path-and-kwargs.patch` | 10 | INTACT 6, BROKEN_BY_BRANCH 1, INCONCLUSIVE 3 | INTACT 6, BROKEN_BY_BRANCH 1, CASCADE 3 |
| `0017-asr-vanilla-base.patch` | 9 | INCONCLUSIVE 9 | INTACT 6, CASCADE 3 |
| `0018-asr-detected-language.patch` | 5 | INTACT 2, BROKEN_BY_BRANCH 2, INCONCLUSIVE 1 | INTACT 2, BROKEN_BY_BRANCH 2, CASCADE 1 |
| `0020-subgen-kwargs-json-parse.patch` | 1 | INCONCLUSIVE 1 | INTACT 1 |
| `0021-entrypoint-device-guard.patch` | 1 | SKIPPED_OTHER_FILE 1 | INTACT 1 |
| `0022-runtime-config-endpoint.patch` | 4 | INTACT 1, INCONCLUSIVE 3 | INTACT 1, CASCADE 3 |
| `0023-expose-concurrent-transcriptions.patch` | 1 | INCONCLUSIVE 1 | CASCADE 1 |
| `0024-per-request-ignore-forced.patch` | 14 | INTACT 4, INCONCLUSIVE 10 | INTACT 12, CASCADE 2 |
| `0025-direct-path-containment-guard.patch` | 4 | INTACT 1, INCONCLUSIVE 3 | INTACT 4 |
| `0026-detect-language-track-param.patch` | 5 | INTACT 1, INCONCLUSIVE 4 | INTACT 5 |
| `0027-config-live-model-and-swap-lock.patch` | 5 | INTACT 1, INCONCLUSIVE 4 | INTACT 1, CASCADE 4 |
| `0028-bump-patch-rev-v4.16.patch` | 1 | INCONCLUSIVE 1 | CASCADE 1 |
| `0029-async-config-switch.patch` | 6 | INCONCLUSIVE 6 | INTACT 1, CASCADE 5 |
| `0030-bump-patch-rev-v4.17.patch` | 1 | INCONCLUSIVE 1 | CASCADE 1 |
| `0031-fix-robust-detect-probability.patch` | 1 | INCONCLUSIVE 1 | INTACT 1 |
| `0032-bump-patch-rev-v4.18.patch` | 1 | INCONCLUSIVE 1 | CASCADE 1 |
| `0034-bump-patch-rev-v4.19.patch` | 1 | INCONCLUSIVE 1 | CASCADE 1 |
| `0035-bump-patch-rev-v4.20.patch` | 1 | INCONCLUSIVE 1 | CASCADE 1 |
| `0036-bump-patch-rev-v4.21.patch` | 1 | INCONCLUSIVE 1 | CASCADE 1 |
...
2. ~~**The INCONCLUSIVE 86 are still unresolved.**~~ **Resolved 2026-08-07 by the cumulative-apply run.** They split 45 INTACT and 41 CASCADE, with **zero new decisive breaks**. The 11 were never a lower bound; they are the complete census, and every one of the 42 cascades is attributable to one of those 11 rather than being independent damage. The machine section above carries the resolved tally. Nine of the cascades are version-string bumps (`0028` through `0036`) landing on the line `0009`'s file header edits, which is a good illustration of why a cascade must not be counted as damage.

   This manual pass still covers only the four items that could have stopped Phase 2. That scope is unchanged and deliberate; what changed is that the unexamined remainder is no longer unexamined.
